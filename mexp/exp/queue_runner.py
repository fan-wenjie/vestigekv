...
def run_client(job, port):
    ...
    if client == "ruler":
        ...
        elif naming._checkpoint(job):
            # A non-default checkpoint needs its own name here too. run_ruler.py
            # builds both filenames from --arm alone, so without this the Base
            # pair writes results_ and samples_ under the Instruct names. It did:
            # on 2026-09-21 the Base run overwrote samples_{baseline,vestigekv}
            # _n50_4096-8192-16384-32768-65536.json, and only the results_ files
            # were separated afterwards. naming.ruler_out already expects the
            # marker, so the audit and the client disagreed about the path.
            cmd += ["--tag", naming._checkpoint(job).lstrip("_")]
        ...
        if "limit" in args:
            ...
            # A non-default checkpoint has to change the record's name. Without
            # it a Base run writes summary_baseline.json, which IS the Instruct
            # run's file, and the second silently becomes the first -- the
            # failure naming._checkpoint exists to prevent. Every other client
            # gets the marker from there; this one was never wired to it, and
            # audit_queue computes no output path for longbench1, so nothing
