...
ONE KNOB TO KNOW ABOUT. Both arms captured a cuda graph for bs=2 while
decoding at bs=1. That is a dispatch floor VestigeKV's per-step kernels pay and
dense mostly does not, so if it biases the ratio it biases it downward. Both
arms carry it, so the comparison stays controlled; the printed number is the
conservative side of the bias.
"""
...
LOGS = {
    "dense": "server_baseline_paired512k-dense.log",
    "vestigekv": "server_vestigekv_paired512k-vestigekv.log",
}
HALFWIDTH = 2048
...
def main():
    ...
    body = [
        ...
        f"% results/kimi/server_{{baseline_paired512k-dense,vestigekv_paired512k-vestigekv}}.log",
