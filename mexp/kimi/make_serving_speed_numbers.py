...
def main():
    ...
    # hand-written macro comment and had no derivation -- the run reaches
    # 524288, whose window is half empty because the run stops there, and
    # widening or one-siding the rule for a single point is the kind of
    # special case a reader is right to distrust.
    top = min(max(n for n, _ in rows) for rows in arms.values()) - HALFWIDTH
    top -= top % 1024
    ...
    body = [
        ...
        "% Contexts are labelled in units of 1024 tokens. The rightmost row is",
        f"% the last one a +/-{HALFWIDTH}-token window fully covers ({top} tokens);",
        "% the run itself stops at 524288, where the window would be half empty.",
        "",
