...
(and the 128k-1M files once measured; \\PENDING until then).
...
def paired(line, n, lengths):
    ...
    right object, and what it needs is per-item SCORES, not generations. Those
    survive for every arm in paired_scores.json even where the generations do
    not, which on the n=50 grid is both of them.
    ...
    _red = json.load(open(red)) if os.path.exists(red) else {}
    reduced = _red.get("scores", {})
    # The reduction stamps the size of the record it was taken from. A full
    # record that no longer matches its stamp is a different run under the same
    # name, so say so once rather than let a silent fallback decide.
    for _k, _p in _red.get("provenance", {}).items():
        _f = os.path.join(d, _p.get("source", ""))
        if os.path.exists(_f) and os.path.getsize(_f) != _p.get("bytes"):
            print(f"   note: {_p['source']} is {os.path.getsize(_f)} bytes, "
                  f"stamped {_p['bytes']}; using the reduction for {_k}")

    def scores(arm):
        """{task: {"<doc_id>,<length>": score}}, reduction or full record.
        ...
        The REDUCTION is preferred, and that is not a size preference. A
        samples_ filename carries the arm but no checkpoint marker, so a Base
        run overwrites the Instruct file of the same shape; on 2026-09-21 it
        did, and the full records at those paths have held Base data under
        Instruct names ever since. The reduction was taken before that and
        records the byte count it was taken from, so it is the only copy that
        can be checked. A full record whose size disagrees with that stamp is
        not the run this reduction came from, and is refused rather than
        silently preferred."""
        stem = f"{arm}_n{n}_{'-'.join(map(str, lengths))}"
        key = next((k for k in reduced if k.startswith(stem)), None)
        if key:
            return reduced[key]
...
def signflip_p(d):
    ...
    # No hit in `trials` draws does not make the p-value zero; it makes it
    # smaller than this estimator can resolve. Returning 0.0 here printed
    # "p = 0.000", which reads as exact and is not what was measured. The
    # resolution floor is the honest answer, and the caller prints it as a
    # bound rather than a value.
    return max(hit, 1) / trials
...
def main():
    ...
    # sample moves a cell by 0.02), and the long grid at 5.
    for line, tag, n, lengths in (("kimi", "K", 50, [4096, 8192, 16384, 32768, 65536]),
        ...
        if arms["D"] and arms["V"]:
            ...
            if pd:
                ...
                # A sign-flip p of 1.7e-13 printed as "0.000" reads as
                # exactly zero and understates the result by twelve orders of
                # magnitude. Below 0.001 the bound is what is informative, and
                # the macro is only ever used inside math mode.
                _p = max(signflip_p(pd), 1e-12)
                _ps = (f"{_p:.3f}" if _p >= 1e-3
                       else f"{{<}}10^{{{math.floor(math.log10(_p)) + 1}}}")
                L.append(f"\\newcommand{{\\ruler{tag}PairedP}}{{{_ps}}}")
    ...
    if os.path.exists(_xp):
        _vestigekv_extract = {k: v for k, v in json.load(open(_xp)).items()
                       if not k.startswith("_")}

    def vestigekv_stats(job):
        s = os.path.join(ROOT, "results", "kimi", f"server_vestigekv_{job}.log")
        if os.path.exists(s):
            # The tag was renamed VKSTATS -> VESTIGEKV_STATS when the vk prefix
            # was freed for the vulkan backend; archived logs still carry the old
            # one. " steps=" anchors it so the server-args line, which contains
            # SGLANG_DEBUG_VESTIGEKV_STATS, cannot match.
            tags = ("VESTIGEKV_STATS steps=", "VKSTATS steps=")
            last = [
                l
                for l in open(s, errors="replace")
                if any(t in l for t in tags) and "TP1]" not in l
            ]
        ...
        return _vestigekv_extract.get(job)
