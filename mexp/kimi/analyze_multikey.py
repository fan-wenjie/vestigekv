...
def classify(path, threshold, lengths=None):
        ...
        for s in recs:
            for k, v in s.items():
                if not (k.isdigit() and isinstance(v, (int, float)) and v >= 0):
                    continue
                # A length filter, so a full-grid record can be read at the
                # same lengths a targeted run used. Without it the Base grid
                # (4k-64k) and the Instruct copy-fidelity run (32k and 64k)
                # answer the same macro names over different populations.
                if lengths and int(k) not in lengths:
                    continue
...
def main():
    ...
    ap.add_argument("--lengths", default="",
                    help="comma list; default every length in the record")
    ap.add_argument("--emit", default="",
                    help="write the mk3-copyfidelity macros to this .tex file")
    args = ap.parse_args()
    LEN = {int(x) for x in args.lengths.split(",") if x.strip()}
    ...
    for p in paths:
        rows = classify(p, args.threshold, LEN)
        print(f"\n## {os.path.basename(p)}")
    ...
    if args.emit:
        # The Instruct numbers come from the mk3-copyfidelity-n50 pair: two
        # tasks, two arms, 100 items each. Emitting from anything else would
        # put a number in the paper that the sentence around it does not
        # describe.
        #
        # The Base numbers come from the full n=50 grid RESTRICTED to the same
        # two lengths, because the targeted run was never repeated on that
        # checkpoint. Those records sit under the Instruct filenames: the Base
        # pair overwrote them on 2026-09-21 (run_ruler.py named samples_ from
        # --arm alone), so the file called samples_vestigekv_n50_4096-... holds
        # Base generations. The restriction is what makes the two comparable,
        # and it is the reason --lengths exists.
        CF = (32768, 65536)
        want = {"baseline": None, "vestigekv": None}
        base = {"baseline": None, "vestigekv": None}
        for p in paths:
            for arm in want:
                if f"samples_{arm}_n50_32768-65536_mk3-copyfidelity" in p:
                    want[arm] = classify(p, args.threshold, set(CF))
                elif p.endswith(f"samples_{arm}_n50_4096-8192-16384-32768-65536.json"):
                    base[arm] = classify(p, args.threshold, set(CF))
        ...
        L = ["% Generated by mexp/kimi/analyze_multikey.py --emit. Do not edit by hand.",
             f"% Both checkpoints restricted to lengths {CF}."]
        def rows(src, tag):
            out = []
            for arm, a in (("vestigekv", "Vk"), ("baseline", "Dense")):
                if src[arm] is None:
                    continue
                for task, short in (("niah_multikey_3", "MKc"), ("niah_single_3", "Sc")):
                    t = [r for r in src[arm] if r[0] == task]
                    if not t:
                        continue
                    out += [f"\\newcommand{{\\cf{a}{tag}{short}N}}{{{len(t)}}}",
                            f"\\newcommand{{\\cf{a}{tag}{short}Ok}}{{{sum(r[3]=='ok' for r in t)}}}",
                            f"\\newcommand{{\\cf{a}{tag}{short}Near}}{{{sum(r[3]=='near' for r in t)}}}",
                            f"\\newcommand{{\\cf{a}{tag}{short}Wrong}}{{{sum(r[3]=='wrong' for r in t)}}}"]
            return out
        L += rows(want, "")
        L += rows(base, "Base")
