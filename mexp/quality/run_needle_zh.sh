#!/bin/bash
# Serving needle only, 4 cells (Base/Instruct x vestigekv/dense), ctx 139264
# (needle prompt = 131072 document + question; server ctx must exceed it).
...
run_cell() {
  ...
  local log="runs/$DATE/server_needle_zh_${tag}.log"
  ...
  echo "== CELL $tag: needle gen =="
  $PY mexp/quality/needle_chinese.py gen 131072 8 \
    "results/quality_needle_zh_${tag}.json" > "runs/$DATE/qgate_needle_zh_${tag}.log" 2>&1
...
for sfx in "" "_instruct"; do
  $PY mexp/quality/needle_serving.py compare \
    "results/quality_needle_zh_vestigekv${sfx}.json" \
    "results/quality_needle_zh_dense${sfx}.json" \
    "results/quality_needle_zh_verdict${sfx}.json"
done
echo ALL_DONE
