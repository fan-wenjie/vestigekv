#!/bin/bash
# Serving needle n=8 -> n=32 extension (pre-registered: n fixed at 32 before
# the run, all trials reported). Same frozen protocol as run_qgates.sh /
# run_needle.sh: 4 cells (Base/Instruct x vestigekv/dense), ctx 139264,
# radix OFF, graph bs<=2, vk arm threshold 0, client serial, greedy.
# Trial seeds derive per-trial (SEED*1000+trial), so the first 8 trials
# reproduce the n=8 run's prompts exactly; the filler cache rebuilds to
# 32*131072 tokens on first cell (deterministic, same stream).
set -uo pipefail
cd "$(dirname "$0")/../.."
...
run_cell() {
  ...
  local log="runs/$DATE/server_needle32_${tag}.log"
  ...
  echo "== CELL $tag: needle gen n=32 =="
  $PY mexp/quality/needle_serving.py gen 131072 32 \
    "results/quality_needle32_${tag}.json" > "runs/$DATE/needle32_${tag}.log" 2>&1
...
for sfx in "" "_instruct"; do
  $PY mexp/quality/needle_serving.py compare \
    "results/quality_needle32_vestigekv${sfx}.json" \
    "results/quality_needle32_dense${sfx}.json" \
    "results/quality_needle32_verdict${sfx}.json"
done
echo ALL_DONE
