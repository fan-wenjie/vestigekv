...
def bench_point(S, layers, bs, dev="cuda"):
    ...
    if S < D.ACTIVATION_MIN_TOKENS:
        # Serving semantics below the activation threshold: kept == every row,
        # no tiers are built, the pack carries no pairs (the baked kernels
        # early-exit on a_len==0, ~5 us flat in the server profile -- below
        # this bench's resolution). Attention itself is byte-identical to
        # dense, so the method's term is the dense step.
        return t(dense_step), t(dense_step)
