...
ARCH = {
    # trust_remote_code needs PLAIN dirs (real *.py files, symlinked weights),
    # not HF-cache snapshot dirs: the dynamic-module hash walk resolves a
    # snapshot symlink into blobs/ and then cannot find its relative imports.
    # Rebuild base_plain/instruct_plain: symlink every non-*.py file from the
    # HF snapshot, cp -L the *.py.
    "kimi": dict(
        path="~/.cache/vestigekv/base_plain",
        attn_cls="KimiMLAAttention", pe="nope"),
    "kimi_instruct": dict(
        path="~/.cache/vestigekv/instruct_plain",
        attn_cls="KimiMLAAttention", pe="nope"),
    ...
    # Rebuild: from the Instruct snapshot symlink every file except *.py and
    # model-*.safetensors/model.safetensors.index.json, cp -L the *.py (plain
    # files, see above), then symlink model-*.safetensors + the index from the
    # Base snapshot (modeling_kimi.py DIFFERS between the two; the index is
    # byte-identical).
    "kimi_base_mirror": dict(
        path="~/.cache/vestigekv/base_mirror",
        attn_cls="KimiMLAAttention", pe="nope"),
    "dsv2": dict(
