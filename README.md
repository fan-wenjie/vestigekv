...
| `mexp/exp/latency-pair-512k.sh` | the bs=1 latency curve with both arms on the released tree in one sitting, server log KEPT. The paper's 1.28x/1.53x are hand-maintained values naming server logs that no longer exist, and the client JSONL that does ship reduces to 0.066 at 508k -- VestigeKV 15x slower where the abstract claims 1.53x faster -- because the streaming client's ITL is contaminated above ~80-180 tok/s and the contamination penalises the faster arm. This run produces a pair that exists, shares a tree, and ships |
| `paired512k-bs1-{dense,vestigekv}` (queue) | the same pair again at `GRAPH_BS=1`. The first attempt captured a graph for bs=2 while decoding at bs=1, breaking the `GRAPH_BS == MAX_REQS` convention 128 of the other 130 queue rows keep. Its dense arm reproduced two independent earlier records at 256k (5.571 against 5.590 and 5.598) while its vestigekv arm came out 3.7% slower than both (4.508 against 4.364 and 4.345), which is what a surplus graph lane looks like: dispatch the six per-step VestigeKV kernels pay and the dense path does not. The first pair is retired, not deleted, under `~/vestigekv-retired/graphbs2-latency-pair/`, and `mexp/exp/audit_queue.py` now refuses a job whose captured graph is wider than its batch |
| `mexp/kimi/sidecar_spectrum_offline.py` (analysis, waits on `sidecardump-vestigekv`) | whether tier-1's low-pass misses periodic structure the real sidecar carries -- the part of the pre-review question no algebra settles. Reports, per snapshot, the share of energy above the kappa cutoff and the peakiness of what is there. The share is not diagnostic on its own: kappa=16 of 2048 bins keeps so little of a white spectrum that broadband noise already reads 99%. Peakiness is, because only a component shared across rows inflates every sigma alike and so cannot separate them. Driven with planted data before use: broadband reads 1.4, a periodic component at bin 200 reads 9224. It also refuses a snapshot whose `row_slots` are non-monotone, because pool order is the allocator's business and a shuffled block makes a token-axis spectrum meaningless while looking entirely normal |
| `sidecardump-vestigekv` (queue, tail) | the sidecar rows themselves, to answer a pre-review question on tier-1's low-pass with data rather than algebra. The reviewer asked whether pairing feature dims into complex numbers and low-passing the complex spectrum would work better, by analogy with RoPE. Two things answer it without a run: the branch is un-roped by construction (`SIDECAR_DIM`: "Un-roped half of the latent row"), which is the paper's own NoPE argument, so there is no rotation to exploit; and a symmetric complex low-pass is algebraically the current per-channel real low-pass -- measured identical to 8.9e-16, and unchanged when the pairing is permuted, which is the lock: a transform that used the pairing could not be indifferent to it. What is NOT answered is the question underneath: whether the low-pass misses periodic structure real keys carry. `SGLANG_DEBUG_VESTIGEKV_DUMP_DIR` already writes whole 576-dim pool rows, so `rows[:, 512:576]` is the sidecar and no new instrumentation is needed. One request, one task, 65536 tokens, about 0.5 GB of `.pt` -- a study input, not a shipped record |
| `rulern50base-{dense,vestigekv}` (queue, tail) | the RULER row of Table~\ref{tab:capability}, which compares both checkpoints and shows `---` in the two Base columns while GSM8K and MAUVE have them. The Base weights are already local and `common.sh` honours `MODEL`, so only the client was hardcoded to Instruct -- it would have asked a Base server for Instruct and labelled the record with a checkpoint it did not run. Records now carry a checkpoint marker in their name, because a Base run and an Instruct run at the same n and lengths otherwise write one file. Same protocol as the Instruct row, n=50, seed 0, about 2.3 h per arm |
| `tputB-bs32mf087/mf085-vestigekv` (queue) | whether the 128k top point is a limit of the method or of one deployment knob. VestigeKV OOMs there twice, reproducibly, at 31 live requests and 0.82 pool occupancy -- and the allocation that fails is 20 MiB, outside a KV pool both arms size identically at 37.70 GB. `expandable_segments` is already on, so this is not fragmentation the allocator can absorb. What differs is headroom: after startup dense has 9.02 GB free and VestigeKV 8.22, a 0.80 GB static footprint the paper already accounts for. These two rerun the point at `mem_fraction_static` 0.87 and 0.85. If it completes, the limitation is 'needs a smaller static fraction at this shape', not 'cannot run' -- a materially different sentence, and the wrong one is the one currently supported by the evidence |
| `tputB-bs32r2-{dense,vestigekv}` (queue, tail) | one repeat of the 128k top point, because `tputB-bs32-vestigekv` died there and a limitation claimed from a single crash is not a limitation. The dense arm completed the same point; VestigeKV's rank 0 was killed at 31 live requests and 82% pool occupancy, with an identical backend configuration to the 24-live run that survived, so the allocation that failed is outside the KV pool and scales with live requests times rows. If it reproduces it belongs in Limitations; if it does not, nothing is written |
...
```json
{"args":{"concurrency":32,"input_len":65536,"num_prompts":32,"output_len":4096,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"32","MAMBA_SLOTS":"33","MAX_REQS":"32"},"id":"diag-slots33-bs32-dense","probe":false}
{"args":{"concurrency":4,"input_len":131072,"num_prompts":4,"output_len":16384,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"135168","GRAPH_BS":"32","MAMBA_SLOTS":"33","MAX_REQS":"32"},"id":"diag-long128k-bs4-dense","probe":false}
...
{"args":{"concurrency":1,"input_len":65536,"num_prompts":5,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs1-dense","probe":false}
{"args":{"concurrency":1,"input_len":65536,"num_prompts":5,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs1-vestigekv","probe":false}
{"args":{"concurrency":2,"input_len":65536,"num_prompts":8,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs2-dense","probe":false}
{"args":{"concurrency":2,"input_len":65536,"num_prompts":8,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs2-vestigekv","probe":false}
{"args":{"concurrency":4,"input_len":65536,"num_prompts":12,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs4-dense","probe":false}
{"args":{"concurrency":4,"input_len":65536,"num_prompts":12,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs4-vestigekv","probe":false}
{"args":{"concurrency":8,"input_len":65536,"num_prompts":16,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs8-dense","probe":false}
{"args":{"concurrency":8,"input_len":65536,"num_prompts":16,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs8-vestigekv","probe":false}
{"args":{"concurrency":12,"input_len":65536,"num_prompts":24,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs12-dense","probe":false}
{"args":{"concurrency":12,"input_len":65536,"num_prompts":24,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs12-vestigekv","probe":false}
{"args":{"concurrency":16,"input_len":65536,"num_prompts":16,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs16-dense","probe":false}
{"args":{"concurrency":16,"input_len":65536,"num_prompts":16,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs16-vestigekv","probe":false}
{"args":{"concurrency":24,"input_len":65536,"num_prompts":24,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs24-dense","probe":false}
{"args":{"concurrency":24,"input_len":65536,"num_prompts":24,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs24-vestigekv","probe":false}
{"args":{"concurrency":32,"input_len":65536,"num_prompts":32,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs32-dense","probe":false}
{"args":{"concurrency":32,"input_len":65536,"num_prompts":32,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"74752","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputA-bs32-vestigekv","probe":false}
{"args":{"concurrency":1,"input_len":131072,"num_prompts":5,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs1-dense","probe":false}
{"args":{"concurrency":1,"input_len":131072,"num_prompts":5,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs1-vestigekv","probe":false}
{"args":{"concurrency":2,"input_len":131072,"num_prompts":8,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs2-dense","probe":false}
{"args":{"concurrency":2,"input_len":131072,"num_prompts":8,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs2-vestigekv","probe":false}
{"args":{"concurrency":4,"input_len":131072,"num_prompts":12,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs4-dense","probe":false}
{"args":{"concurrency":4,"input_len":131072,"num_prompts":12,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs4-vestigekv","probe":false}
{"args":{"concurrency":8,"input_len":131072,"num_prompts":16,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs8-dense","probe":false}
{"args":{"concurrency":8,"input_len":131072,"num_prompts":16,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs8-vestigekv","probe":false}
{"args":{"concurrency":12,"input_len":131072,"num_prompts":24,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs12-dense","probe":false}
{"args":{"concurrency":12,"input_len":131072,"num_prompts":24,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs12-vestigekv","probe":false}
{"args":{"concurrency":16,"input_len":131072,"num_prompts":16,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs16-dense","probe":false}
{"args":{"concurrency":16,"input_len":131072,"num_prompts":16,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs16-vestigekv","probe":false}
{"args":{"concurrency":24,"input_len":131072,"num_prompts":24,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs24-dense","probe":false}
{"args":{"concurrency":24,"input_len":131072,"num_prompts":24,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs24-vestigekv","probe":false}
{"args":{"concurrency":32,"input_len":131072,"num_prompts":32,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs32-dense","probe":false}
{"args":{"concurrency":32,"input_len":131072,"num_prompts":32,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs32-vestigekv","probe":false}
{"args":{"concurrency":32,"input_len":65536,"num_prompts":32,"output_len":4096,"rep":2,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"40","MAMBA_SLOTS":"40","MAX_REQS":"40"},"id":"diag-slots40-bs32-dense","probe":false}
...
{"args":{"lengths":"16384,32768,65536","n":50,"seed":0,"tasks":"niah_multikey_2,niah_multikey_3,ruler_qa_hotpot"},"arm":"vestigekv","client":"ruler","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"4","MAMBA_SLOTS":"4","MAX_REQS":"4"},"id":"fbablate-on","probe":false}
{"args":{"lengths":"16384,32768,65536","n":50,"seed":0,"tasks":"niah_multikey_2,niah_multikey_3,ruler_qa_hotpot"},"arm":"vestigekv","client":"ruler","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"4","MAMBA_SLOTS":"4","MAX_REQS":"4","SGLANG_DEBUG_VESTIGEKV_NO_OVERFLOW_FALLBACK":"1"},"id":"fbablate-off","probe":false}
{"args":{"input_len":4096,"output_len":126976,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"135168","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1","SGLANG_DEBUG_VESTIGEKV_STATS":"1"},"id":"fbstream-on","probe":false}
{"args":{"input_len":4096,"output_len":126976,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"135168","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1","SGLANG_DEBUG_VESTIGEKV_NO_OVERFLOW_FALLBACK":"1","SGLANG_DEBUG_VESTIGEKV_STATS":"1"},"id":"fbstream-off","probe":false}
{"args":{"lengths":"131072,262144","n":5,"seed":0},"arm":"baseline","client":"ruler","env":{"CHUNK":"4096","CTX":"270336","GRAPH_BS":"2","MAMBA_SLOTS":"2","MAX_REQS":"2"},"id":"rulerlong-baseline","probe":false}
{"args":{"lengths":"65536","n":10,"seed":0},"arm":"vestigekv","client":"ruler","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"4","MAMBA_SLOTS":"4","MAX_REQS":"4","SGLANG_DEBUG_VESTIGEKV_STATS":"1"},"id":"statsruler64-vestigekv","probe":false}
{"args":{"lengths":"131072,262144","n":5,"seed":0},"arm":"vestigekv","client":"ruler","env":{"CHUNK":"4096","CTX":"270336","GRAPH_BS":"2","MAMBA_SLOTS":"2","MAX_REQS":"2"},"id":"rulerlong-vestigekv","probe":false}
...
{"args":{"lengths":"65536","n":1,"seed":0,"tasks":"niah_multikey_3"},"arm":"vestigekv","client":"ruler","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"4","MAMBA_SLOTS":"4","MAX_REQS":"4","SGLANG_DEBUG_VESTIGEKV_DUMP_DIR":"/home/user/vestigekv/results/kimi/sidecardump"},"id":"sidecardump-vestigekv","probe":false}
...
{"args":{"lengths":"65536","n":5,"seed":0,"tasks":"niah_multikey_3"},"arm":"vestigekv","client":"ruler","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"4","MAMBA_SLOTS":"4","MAX_REQS":"4","SGLANG_DEBUG_VESTIGEKV_DUMP_DIR":"/home/user/vestigekv/results/kimi/sidecardump5"},"id":"sidecardump5-vestigekv","probe":false}
{"args":{"input_len":65536,"num_prompts":5,"output_len":4096},"arm":"vestigekv","client":"continue","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1","SGLANG_DEBUG_VESTIGEKV_DUMP_DIR":"/home/user/vestigekv/results/kimi/sidecardump_prose5"},"id":"sidecardumpprose5-vestigekv","probe":false}
...
{"args":{"lengths":"16384,32768,65536","n":10,"seed":0,"tasks":"niah_single_1,niah_multikey_2,niah_multikey_3,ruler_fwe,ruler_qa_hotpot"},"arm":"vestigekv","client":"ruler","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"4","MAMBA_SLOTS":"4","MAX_REQS":"4","SGLANG_DEBUG_VESTIGEKV_STATS":"1"},"id":"margin0-vestigekv","probe":false,"server_args":["--vestigekv-recall-margin","0"]}
{"args":{"lengths":"16384,32768,65536","n":10,"seed":0,"tasks":"niah_single_1,niah_multikey_2,niah_multikey_3,ruler_fwe,ruler_qa_hotpot"},"arm":"vestigekv","client":"ruler","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"4","MAMBA_SLOTS":"4","MAX_REQS":"4","SGLANG_DEBUG_VESTIGEKV_STATS":"1"},"id":"margin2-vestigekv","probe":false,"server_args":["--vestigekv-recall-margin","2"]}
{"args":{"concurrency":32,"input_len":131072,"num_prompts":32,"output_len":8192,"rep":2,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs32r2-dense","probe":false}
{"args":{"input_len":63488,"num_prompts":1,"output_len":4096},"arm":"vestigekv","client":"continue","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1","SGLANG_DEBUG_VESTIGEKV_DUMP_DIR":"/home/user/vestigekv/results/kimi/sidecardump_prose5"},"id":"sidecardumpprose62k-vestigekv","probe":false}
{"args":{"input_len":61440,"num_prompts":1,"output_len":4096},"arm":"vestigekv","client":"continue","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1","SGLANG_DEBUG_VESTIGEKV_DUMP_DIR":"/home/user/vestigekv/results/kimi/sidecardump_prose5"},"id":"sidecardumpprose60k-vestigekv","probe":false}
{"args":{"input_len":59392,"num_prompts":1,"output_len":4096},"arm":"vestigekv","client":"continue","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1","SGLANG_DEBUG_VESTIGEKV_DUMP_DIR":"/home/user/vestigekv/results/kimi/sidecardump_prose5"},"id":"sidecardumpprose58k-vestigekv","probe":false}
{"args":{"concurrency":32,"input_len":131072,"num_prompts":32,"output_len":8192,"rep":2,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs32r2-vestigekv","probe":false}
{"args":{"n":1209,"shots":64},"arm":"baseline","client":"gsm8k","env":{"CHUNK":"4096","CTX":"73728","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1"},"id":"gsm8kcount-baseline","probe":false}
{"args":{"concurrency":24,"input_len":131072,"num_prompts":24,"output_len":8192,"rep":2,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32"},"id":"tputB-bs24r2-vestigekv","probe":false}
...
{"args":{"input_len":32768,"output_len":4096,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"512","CTX":"135168","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1","MEM_FRAC":"0.96"},"id":"diag-smoke32k4k-baseline","probe":true}
{"args":{"input_len":32768,"output_len":4096,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"512","CTX":"135168","GRAPH_BS":"1","MAMBA_SLOTS":"1","MAX_REQS":"1","MEM_FRAC":"0.96"},"id":"diag-smoke32k4k-vestigekv","probe":true}
...
{"args":{"concurrency":2,"input_len":131072,"num_prompts":8,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs2-vestigekv","probe":false}
{"args":{"concurrency":4,"input_len":131072,"num_prompts":12,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs4-dense","probe":false}
{"args":{"concurrency":4,"input_len":131072,"num_prompts":12,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs4-vestigekv","probe":false}
{"args":{"concurrency":8,"input_len":131072,"num_prompts":16,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs8-dense","probe":false}
{"args":{"concurrency":8,"input_len":131072,"num_prompts":16,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs8-vestigekv","probe":false}
{"args":{"concurrency":12,"input_len":131072,"num_prompts":24,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs12-dense","probe":false}
{"args":{"concurrency":12,"input_len":131072,"num_prompts":24,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs12-vestigekv","probe":false}
{"args":{"concurrency":16,"input_len":131072,"num_prompts":16,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs16-dense","probe":false}
{"args":{"concurrency":16,"input_len":131072,"num_prompts":16,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs16-vestigekv","probe":false}
{"args":{"concurrency":24,"input_len":131072,"num_prompts":24,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs24-dense","probe":false}
{"args":{"concurrency":24,"input_len":131072,"num_prompts":24,"output_len":8192,"seed":0},"arm":"vestigekv","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs24-vestigekv","probe":false}
{"args":{"concurrency":32,"input_len":131072,"num_prompts":32,"output_len":8192,"seed":0},"arm":"baseline","client":"stream","env":{"CHUNK":"4096","CTX":"140288","GRAPH_BS":"32","MAMBA_SLOTS":"32","MAX_REQS":"32","MEM_FRAC":"0.87"},"id":"tputB87-bs32-dense","probe":false}
