...
| `mexp/exp/ruler-fallback-on-control.sh` | the control the two no-fallback RULER arms never had. `trunc-prefix-nofb` and `trunc-spread-nofb` are matched to each other but their fallback-ON counterpart on disk is untagged, with no surviving queue entry naming its tree or flags. This re-runs it with the fallback on and every other knob copied from the nofb queue line, so the review's question -- how much accuracy the dense fallback sustains -- is answered between arms that differ in one flag |
