...
1. **Latency**: 4k prefill -> 512k continuous decode; per-token latency vs S.
   Bar: crossover ~28k; **1.28x at 256k, 1.46x at 496k**; vestigekv slope
   ~1.5 ns/token vs dense ~6.3 ns/token (2026-09-12 server-side logs;
   hardware-dependent, so direction and ordering are the real bar).
2. **Throughput**: 64k prefill + 4k decode, batch 1..16. Bar: within
   launch-noise at bs=1, **+12.4% at bs=12** (attention cannot batch,
...
```
Bar: crossover ~28k; 1.28x at 256k; 1.46x at 496k (2026-09-12 server-side logs).
...
```
Bar: launch-noise at bs=1, +12.4% at bs=12.
