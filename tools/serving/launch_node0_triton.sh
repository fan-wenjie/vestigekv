...
# code-sync guard: refuse to launch if the attention backend differs across nodes
L=$(md5sum /home/user/fft/sglang/python/sglang/srt/layers/attention/vestigekv_mla_backend.py | cut -d" " -f1)
R=$(ssh gpu5090 "md5sum /home/user/fft/sglang/python/sglang/srt/layers/attention/vestigekv_mla_backend.py 2>/dev/null | cut -d\" \" -f1" 2>/dev/null)
[ -n "$L" ] && [ -n "$R" ] || { echo "ABORT: md5 empty -- guard would be checking nothing"; exit 1; }
[ "$L" = "$R" ] || { echo "ABORT: vestigekv backend differs across nodes (sync first)"; exit 1; }
