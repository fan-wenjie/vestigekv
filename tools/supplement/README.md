...
| `engine/vestigekv.patch` | the whole method: 32 files, ~10.3k lines added, 4 deleted |
...
diff` against sglang commit `17ba2c2e7c7b81f31a8a9e693e7435ab262c16b4`, fetched
by `setup_engine.sh` from the official repository. So what you build is upstream
code plus a diff you can read end to end — the method is additive, and the four
deleted lines are the registration hook it replaces.
...
did not measure. The GLM-5.3 line in `mexp/glm53/` is a port in progress and is
not the basis of any claim in the paper.
